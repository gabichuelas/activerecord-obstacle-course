...
# ----------------------
  ...
  it '22. calculates the total sales for all but one user' do
    ...
    # ------------------ Using ActiveRecord ---------------------
    # Solution goes here
    total_sales = Order.where.not(user_id: @user_2.id).sum(:amount).to_i
    # -----------------------------------------------------------
  ...
  it '24. returns all orders for user 2 which include item_4' do
    ...
    # ------------------ Improved Solution ----------------------
    #  Solution goes here
    # orders = Order.joins(:items).where("items.id = ? and user_id = ?", @item_4.id, @user_2.id)
    orders = Order.joins(:items).where("items.id = ?", @item_4.id).where(user_id: @user_2.id)
    # -----------------------------------------------------------
