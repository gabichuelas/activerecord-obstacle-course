...
# ----------------------
  ...
  it '16. returns the names of users who ordered one specific item' do
    ...
    users = User.joins(:orders).joins(:order_items).where('order_items.item_id = ?',  @item_8.id).order('users.name').distinct.pluck('users.name')
    # this works, but NEED PRACTICE WITH AR JOINS
  ...
  it '18. returns the names of items for a users order' do
    ...
    items_for_user_3_third_order = Order.joins(:items).where('orders.user_id = ?', @user_3.id).distinct[2].items.pluck(:name)
